...
namespace Aite.Tools
{
    ...
    {
        public bool selfTypingSound = false;
        public bool TypeSoundPlay => SettingManager.Instance.TypingPlaySound & selfTypingSound;
        ...
        public void OutputText(string text, Action<TypewriterState> onOutputEnd = null)
        {
            ...
            if (text.Length > 0) _textComponent.text = text;
        ...
        /// 输出原先的文字
        /// </summary>
        public void OutputText(Action<TypewriterState> onOutputEnd = null)
        {
            // 如果当前正在执行字符输出，将其中断
            if (State == TypewriterState.Outputting)
            {
        ...
        private IEnumerator OutputCharactersNoFading(bool skipFirstCharacter = true)
        {
            ...
            if (TypeSoundPlay && AudioManager.Instance) AudioManager.Instance.PlayOneShot(AudioManager.ClipEnum.Type);
            ...
            while (_textComponent.maxVisibleCharacters < textInfo.characterCount)
            {
                ...
                if (timer >= interval)
                {
                    ...
                    if(TypeSoundPlay && AudioManager.Instance) AudioManager.Instance.PlayTypeShow();
                }
                yield return null;
            }
